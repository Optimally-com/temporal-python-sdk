...
class Workflow:
    ...
    def current_time_millis() -> int:
        ...
        task: ITask = ITask.current()
        return int(
            task.decider.decision_context.current_time_millis().timestamp() * 1000
        )
...
class WorkflowClient:
    ...
    async def start(cls, stub_fn: Callable, *args) -> WorkflowExecutionContext:
        ...
        return await exec_workflow(
            client, method, args, workflow_options=options, stub_instance=stub
        )
    ...
    async def wait_for_close(self, context: WorkflowExecutionContext) -> object:
        return await self.wait_for_close_with_workflow_id(
    ...
    async def wait_for_close_with_workflow_id(
        self,
        workflow_id: str,
        run_id: str = None,
        ...
        while True:
            ...
            history_response = await self.service.get_workflow_execution_history(
            ...
            if (
                ...
                completed_attributes = (
                    history_event.workflow_execution_completed_event_attributes
                )
                type_hints = []
                if workflow_method_instance:
                    type_hints = workflow_method_instance._ret_type_hints
                payloads: List[object] = self.data_converter.from_payloads(
                    completed_attributes.result, type_hints=type_hints
                )
                return payloads[0]
            elif (
                history_event.event_type
                ...
                terminated_attributes = (
                    history_event.workflow_execution_terminated_event_attributes
                )
                raise WorkflowExecutionTerminatedException(
                    reason=terminated_attributes.reason,
...
async def exec_workflow(
    ...
    start_request = create_start_workflow_request(
        workflow_client, wm, args, workflow_options
    )
    start_response = await workflow_client.service.start_workflow_execution(
        request=start_request
    )
    execution = WorkflowExecution(
        workflow_id=start_request.workflow_id, run_id=start_response.run_id
    )
    stub_instance._execution = execution  # type: ignore
    return WorkflowExecutionContext(
        workflow_type=wm._name,
        workflow_execution=execution,
        workflow_method_instance=wm,
    )


async def exec_workflow_sync(
    workflow_client: WorkflowClient,
    wm: WorkflowMethod,
    args: List,
    workflow_options: WorkflowOptions = None,
    ...
    execution_context: WorkflowExecutionContext = await exec_workflow(
        workflow_client,
        wm,
        args,
...
async def exec_query(
    workflow_client: WorkflowClient, qm: QueryMethod, args, stub_instance: object = None
):
    assert stub_instance._execution  # type: ignore
    ...
    request.namespace = workflow_client.namespace
    response: QueryWorkflowResponse = await workflow_client.service.query_workflow(
        request=request
    )
    """
    ...
    return workflow_client.data_converter.from_payloads(
        response.query_result, qm.ret_type_hints
    )[0]
...
def create_search_attributes(
    s: Dict[str, object], data_converter: DataConverter
) -> SearchAttributes:
...
def create_start_workflow_request(
    workflow_client: WorkflowClient,
    wm: WorkflowMethod,
    args: List,
    workflow_options: WorkflowOptions = None,
    ...
    if wm._search_attributes:
        start_request.search_attributes = create_search_attributes(
            wm._search_attributes, workflow_client.data_converter
        )
    ...
    if workflow_options:
        ...
        if workflow_options.workflow_id_reuse_policy is not None:
            start_request.workflow_id_reuse_policy = (
                workflow_options.workflow_id_reuse_policy
            )
        ...
        if workflow_options.workflow_execution_timeout is not None:
            start_request.workflow_execution_timeout = (
                workflow_options.workflow_execution_timeout
            )
        ...
        if workflow_options.memo is not None:
            start_request.memo = create_memo(
                workflow_options.memo, workflow_client.data_converter
            )
        if workflow_options.search_attributes is not None:
            start_request.search_attributes = create_search_attributes(
                workflow_options.search_attributes, workflow_client.data_converter
            )

    return start_request


def create_close_history_event_request(
    workflow_client: WorkflowClient, workflow_id: str, run_id: str
) -> GetWorkflowExecutionHistoryRequest:
    ...
    history_request.history_event_filter_type = (
...
def get_workflow_stub_fn(wm: WorkflowMethod):
    async def workflow_stub_fn(self, *args):
        ...
        return await exec_workflow_sync(

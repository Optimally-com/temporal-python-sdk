...
from temporal.api.workflowservice.v1 import (
    WorkflowServiceStub as WorkflowService,
    ...
    try:
        while True:
            ...
            try:
                ...
                polling_request: PollActivityTaskQueueRequest = (
                    PollActivityTaskQueueRequest()
                )
                ...
                logger.error(
                    "Error invoking poll_activity_task_queue: %s", ex, exc_info=True
                )
                continue
            task_token = task.task_token
            if not task_token:
                logger.debug(
                    "PollActivityTaskQueue has no task_token (expected): %s", task
                )
                continue
            ...
            activity_context.activity_task = (
                ActivityTask.from_poll_for_activity_task_response(task)
            )

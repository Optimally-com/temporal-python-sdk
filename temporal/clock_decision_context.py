...
class ClockDecisionContext:
    ...
    def create_timer(self, delay_seconds: int, callback: Callable):
        ...
        if delay_seconds == 0:
            ...
            return None
        firing_time = (
            self.current_time_millis().timestamp() * 1000
        ) + delay_seconds * 1000
        ...
        return TimerCancellationHandler(
            start_event_id=start_event_id, clock_decision_context=self
        )
    ...
    def get_version(
        ...
        def func():
            d: Dict[str, Payloads] = {
                "VERSION": self.decider.worker.client.data_converter.to_payloads(
                    [max_supported]
                )
            }
            return d
    ...
    def validate_version(
        self, change_id: str, version: int, min_supported: int, max_supported: int
    ):
    ...
    def handle_marker_recorded(self, event: HistoryEvent):
        ...
        if SIDE_EFFECT_MARKER_NAME == name:
            ...
            self.version_handler.mutable_marker_results[change_id] = MarkerResult(
                data=data
            )
...
class TimerCancellationHandler:
    ...
    def accept(self, reason: Exception):
        self.clock_decision_context.decider.cancel_timer(
            self.start_event_id,

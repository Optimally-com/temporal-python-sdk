...
def test_create_memo():
    memo = create_memo({"name": "bob", "age": 20}, DEFAULT_DATA_CONVERTER_INSTANCE)
...
def test_create_search_attributes():
    search_attributes = create_search_attributes(
        {"name": "bob", "age": 20}, DEFAULT_DATA_CONVERTER_INSTANCE
    )
...
def test_create_start_workflow_request_override_workflow_options():
    client = WorkflowClient(
        None, "the-namespace", None, DEFAULT_DATA_CONVERTER_INSTANCE
    )
    wm = WorkflowMethod()

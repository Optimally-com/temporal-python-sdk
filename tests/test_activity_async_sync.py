...
class GreetingActivitiesImpl:
    async def compose_greeting(self, sleep_seconds):
        ...
        thread = threading.Thread(
            target=greeting_activities_thread_func,
            args=(Activity.get_task_token(), sleep_seconds),
        )
        thread.start()
...
class GreetingWorkflowImpl(GreetingWorkflow):
    ...
    async def get_greeting(self):
        f: ActivityFuture = Async.function(
            self.greeting_activities.compose_greeting, 10
        )
        await f
        v = f.get_result()
        return v
